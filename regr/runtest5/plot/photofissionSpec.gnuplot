...
plot \
"../build/testPhotofissionSpec.res" using 4:((column(1) == iso && int(column(3)) == eng)?column(5):1/0) with linespoints title sprintf("isotope = %g eng = %g MeV", iso, eng), \
"../baseline/testPhotofissionSpec.res.baseline" using 4:((column(1) == iso && int(column(3)) == eng)?column(5):1/0) with linespoints title sprintf("baseline"), \
"../../runtest2/plot/U-232.txt" using 3:(0.1*column(4)) index engindex with linespoints title sprintf("U-232 energy = %g MeV", energy)
