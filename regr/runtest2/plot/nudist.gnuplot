...
plot "../build/testNuDist.res" using 5:((column(1) == isotope && int(column(3)) == eng && int(column(4)) == nudist)?column(6):1/0) with linespoints title sprintf("isotope (%g) = %g energy = %g MeV, nudist option = %g", isoindex, isotope, eng, nudist), \
     "../baseline/testNuDist.res.baseline" using 5:((column(1) == isotope && int(column(3)) == eng && int(column(4)) == nudist)?column(6):1/0) with linespoints title sprintf("baseline")
