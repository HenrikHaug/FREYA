...
plot "../build/testSpspec.res" using 3:((column(1) == isotope)?column(4):1/0) with linespoints title sprintf("isotope (%g) = %g", isoindex, isotope), \
     "../baseline/testSpspec.res.baseline" using 3:((column(1) == isotope)?column(4):1/0) with linespoints title sprintf("baseline"), \
     "../build/testSpspec.res" using 3:(Watt(1.12082e+00, 3.72278e+00, .0093, $3)) title "Analytical Watt fision spectrum for spontaneous fission in U-232"
