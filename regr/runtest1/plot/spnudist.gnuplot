...
plot "../build/testSpNuDist.res" using 3:((column(1) == isotope)?column(4):1/0) with linespoints title sprintf("isotope(%g) = %g", isoindex, isotope), \
     "../baseline/testSpNuDist.res.baseline" using 3:((column(1) == isotope)?column(4):1/0) with linespoints title sprintf("baseline")
