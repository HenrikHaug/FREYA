...
plot \
"../build/testEngConsAllActTotal.res" using 3:((column(1) == iso && column(2) == engConsEng && column(4) != 0.)?column(4):1/0) with points title sprintf("Total neutron and gamma-ray energy, energy conservation, all actinides, isotope = %g engConsEng = %g MeV", iso, engConsEng), \
"../baseline/testEngConsAllActTotal.res.baseline" using 3:((column(1) == iso && column(2) == engConsEng && column(4) != 0.)?column(4):1/0) with points title sprintf("baseline")
