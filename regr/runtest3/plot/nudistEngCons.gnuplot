...
plot "../build/testEngConsNudist.res" using 5:((column(1) == isotope && int(column(3)) == eng)?column(6):1/0) with linespoints title sprintf("isotope (%g) = %g energy = %g MeV", isoindex, isotope, eng), \
     "../baseline/testEngConsNudist.res.baseline" using 5:((column(1) == isotope && int(column(3)) == eng)?column(6):1/0) with linespoints title sprintf("baseline")
